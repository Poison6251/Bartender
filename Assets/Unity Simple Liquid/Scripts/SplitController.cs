...
namespace UnitySimpleLiquid
{
    ...
    public class SplitController : MonoBehaviour
    {
		...
		public float BottleneckRadius { set { bottleneckRadius = value; } }
        ...
        private Plane GenerateBottleneckPlane()
        {
            ...
            var max = mesh.bounds.max.y + height;
        ...
        private void SplitLogic(Vector3 splitPos)
        {
			...
			// ============================ 추가한 구문 ==========================================
            var lostAmount = liquidContainer.Volume * liquidStep;
			if(liquidContainer.GetFluidFlowManager)
			liquidContainer.GetFluidFlowManager.FluidDrop(lostAmount);
            // ====================================================================================
		...
		private void TransferLiquid(RaycastHit hit, float lostPercentAmount, float scale)
        {
			...
			if (insideRadius)
			{
                ...
                // ============================ 추가한 구문 ==========================================
                liquidContainer.GetFluidFlowManager.Send(liquid.liquidContainer.GetFluidFlowManager, lostAmount);
                // ====================================================================================
                //color change in capacity
                SendLiquidContainer(liquid.liquidContainer,lostAmount);
			}
		...
		private void SendLiquidContainer(LiquidContainer lc,float sendFluidAmount)
		{
			...
			float targetVolume = lc.FillAmount;
			float wholeVolume = targetVolume + sendFluidAmount;
			lc.LiquidColor = Color.Lerp(lc.LiquidColor, newColor,sendFluidAmount/wholeVolume);
		}
		#endregion
        ...
        //{
        //    Debug.Log("충돌");
        //    liquidContainer.FillAmount += particleAmount;
        //}
    }
}